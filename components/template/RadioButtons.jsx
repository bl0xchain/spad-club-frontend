        ...
        <div className="flex">
            {options.map((option, index) => (
                <div key={index} className="flex items-center mr-4">
                    ...
                    <div className='h-4 w-4 absolute rounded-full pointer-events-none peer-checked:border-purple-300 peer-checked:border-2'></div>
                    <label htmlFor={`${name}-${index}`} className="ml-2 text-sm font-medium text-gray-900">{option.value}</label>
                </div>
            ))}
        </div>
    )
